...
export class BoardWorkerComponent extends DestroyComponent implements OnDestroy {
  ...
  constructor(public sudokuLab: SudokuLab,
    ...
    this._manager = new BoardDataManager(board, { saveDataOnChanges: true });
    ...
    this._manager.changed$.pipe(debounceTime(250)).subscribe(() => sudokuLab.state.stepInfos$.next([]));

    _dialog.afterOpened.pipe(takeUntil(this._destroy$)).subscribe(() => this._manager.data.disabled$.next(true));
    _dialog.afterAllClosed.pipe(takeUntil(this._destroy$)).subscribe(() => this._manager.data.disabled$.next(false));
  }

  changed() {
    if (this._worker) this._worker.postMessage(<BoardWorkerArgs>{ sdk: this.board.sdk$.value });
  }
