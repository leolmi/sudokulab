import {forEach as _forEach, includes as _includes, intersection as _intersection, keys as _keys, reduce as _reduce, remove as _remove} from 'lodash';
import {Algorithm} from "../Algorithm";
import {AlgorithmType} from "../enums";
import {PlaySudoku} from "../PlaySudoku";
import {AlgorithmResult, AlgorithmResultLine} from "../AlgorithmResult";
import {Dictionary} from "@ngrx/entity";
import {getAllGroups, getByVisibles, getGroups, getUserCoord, getValues} from "../../sudoku.helper";
  ...
  apply = (sdk: PlaySudoku): AlgorithmResult => {
    ...
    _forEach(sdk.groups, (g1) => {
      ...
      _forEach(g1?.availableOnCells || {}, (cls, v) => {
        // chiavi delle celle che contengono il valore "v"
            ...
            if (gks2.length > 1 && g2.id !== g1?.id) {
              ...
              getAllGroups(sdk, gks2).forEach(g3 => {
                ...
                if (gks3.length === 2 && g3.id !== g2.id && g3.id !== g1?.id && _intersection(gks3, gks1).length === 0) {
                  ...
                  const externals_all: any = _reduce(externals.concat(gks2), (x, id) => ({...x, [id]: true}), {});
                  ...
                  if (groups3.length < 1) {
                    ...
                    const vcells = getByVisibles(sdk, externals).filter(c => !externals_all[c.id] && _includes(c.availables, v));
                    // if (vcells.length>0) {
